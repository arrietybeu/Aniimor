-- chunkname: @C:\\dev\\jkroot\\ws\\workspace\\XWinPublish\\proj\\p4dir\\client\\LuaScripts\\Common\\Data\\AICtrData\\CTGraph\\BC_Wild_GroupBehav_SheepHappyChase.lua

local CTHelper = require("Common.AI.ConditionTrigger.CTHelper")
local _M = {}
local _C = CTHelper.SafeCall
local _P = CTHelper.DoPatrolBehavior
local _A = CTHelper.DoAction
local _B = CTHelper.BeginBehaviourV2

function _M.executeEventTrigger(flow, eventName)
	if eventName == "GBPMsg_ResPoint02" then
		return _M._to_51_0(flow)
	end

	if eventName == "GBPMsg_ResPoint01" then
		flow:setActive()

		local _0 = _M._get_104_2(flow)
		local _1 = _M._get_104_3(flow)

		_A(flow, "PreJoinResPointPort", 0, _0, _1)

		return _M._to_107_0(flow)
	end

	if eventName == "GBPMsg_ResPoint03" then
		return _M._to_112_0(flow)
	end

	if eventName == "GBPMsg_ResPoint04" then
		return _M._to_115_0(flow)
	end

	if eventName == "GBPMsg_ResPoint05" then
		return _M._to_117_0(flow)
	end

	if eventName == "GBPMsg_ResPoint06" then
		return _M._to_120_0(flow)
	end
end

function _M.executeEndTrigger(flow)
	flow:setActive()

	local _0 = _M._get_104_2(flow)
	local _1 = _M._get_104_3(flow)

	_A(flow, "ExitResPointPort", 0, _0, _1, 0, 0)

	return true
end

function _M.executeContinue(flow, nodeId)
	if nodeId == 51 then
		return true
	end

	if nodeId == 107 then
		return true
	end

	if nodeId == 112 then
		return true
	end

	if nodeId == 115 then
		return true
	end

	if nodeId == 117 then
		return true
	end

	if nodeId == 120 then
		return true
	end
end

function _M.checkInterrupt(flow, nodeId)
	return
end

function _M._to_51_0(flow)
	if flow.__isAdditive then
		return
	end

	flow:setActive()

	local _0 = _M._get_41_1(flow)

	if _P(flow, 1, _0, 1, nil) then
		flow:setContinue(51)

		return true
	end
end

function _M._to_107_0(flow)
	if not _B(flow, "PBT_MoveToResPointPortInDist") then
		return
	end

	local _0 = _M._get_104_2(flow)
	local _1 = _M._get_104_3(flow)

	flow.__agent:clearSubTreeLocalParams()
	flow.__agent:addSubTreeLocalParam("tPointId", _0)
	flow.__agent:addSubTreeLocalParam("tPortId", _1)
	flow.__agent:addSubTreeLocalParam("tTimeout", 1000)
	flow.__agent:addSubTreeLocalParam("tSpeedRateType", 2)
	flow.__agent:addSubTreeLocalParam("tSpeed", 0)
	flow.__agent:addSubTreeLocalParam("tInteractDist", 0)
	flow.__agent:addSubTreeLocalParam("tIgnoreSelfBodySize", true)
	flow.__agent:addSubTreeLocalParam("tIgnorePointBodySize", true)
	flow.__agent:addSubTreeLocalParam("tUseAccurateArrive", false)
	flow:setContinue(107)

	return true
end

function _M._to_112_0(flow)
	if flow.__isAdditive then
		return
	end

	flow:setActive()

	local _0 = _M._get_113_1(flow)

	if _P(flow, 1, _0, 1, nil) then
		flow:setContinue(112)

		return true
	end
end

function _M._to_115_0(flow)
	if flow.__isAdditive then
		return
	end

	flow:setActive()

	local _0 = _M._get_116_1(flow)

	if _P(flow, 1, _0, 1, nil) then
		flow:setContinue(115)

		return true
	end
end

function _M._to_117_0(flow)
	if flow.__isAdditive then
		return
	end

	flow:setActive()

	local _0 = _M._get_118_1(flow)

	if _P(flow, 1, _0, 1, nil) then
		flow:setContinue(117)

		return true
	end
end

function _M._to_120_0(flow)
	if flow.__isAdditive then
		return
	end

	flow:setActive()

	local _0 = _M._get_121_1(flow)

	if _P(flow, 1, _0, 1, nil) then
		flow:setContinue(120)

		return true
	end
end

function _M._get_41_1(flow)
	local _0 = flow:getContextValue("tPointId")

	return _C(41, "GetRouteIdFromResPoint", flow, true, _0, 1)
end

function _M._get_104_2(flow)
	return flow:getContextValue("tPointId")
end

function _M._get_104_3(flow)
	return flow:getContextValue("tPortId")
end

function _M._get_113_1(flow)
	local _0 = flow:getContextValue("tPointId")

	return _C(113, "GetRouteIdFromResPoint", flow, true, _0, 2)
end

function _M._get_116_1(flow)
	local _0 = flow:getContextValue("tPointId")

	return _C(116, "GetRouteIdFromResPoint", flow, true, _0, 3)
end

function _M._get_118_1(flow)
	local _0 = flow:getContextValue("tPointId")

	return _C(118, "GetRouteIdFromResPoint", flow, true, _0, 4)
end

function _M._get_121_1(flow)
	local _0 = flow:getContextValue("tPointId")

	return _C(121, "GetRouteIdFromResPoint", flow, true, _0, 5)
end

return _M
