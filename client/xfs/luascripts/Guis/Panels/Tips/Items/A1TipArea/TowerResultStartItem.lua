-- chunkname: @C:\\dev\\jkroot\\ws\\workspace\\XWinPublish\\proj\\p4dir\\client\\LuaScripts\\Guis\\Panels\\Tips\\Items\\A1TipArea\\TowerResultStartItem.lua
...
local TowerResultStartItem = Class.LightClass("TowerResultStartItem", BaseQueueItem)
local Const = require("Common.Const.Const")

function TowerResultStartItem:onInit()
	self:setMaxLimit(1)

	self.uContainer = self.uWidget
end

function TowerResultStartItem:onUpdate()
	self:tryPopupItem()
	self:refreshRemainTime()
end

function TowerResultStartItem:tryPopupItem()
...
function TowerResultStartItem:onClearRunningList(force)
	self:clearRecycleRunningList(force)
end

function TowerResultStartItem:refreshRemainTime()
...
function TowerResultStartItem:initUContainer(data)
...
function TowerResultStartItem:renderItem(item, data)
	local objectReference = item:GetComponent("ObjectReference")
	local textDownUSDFText = objectReference:GetRefValue("textDownUSDFText")
	local textUpUSDFText = objectReference:GetRefValue("textUpUSDFText")

	ClientTextUtils.setText(textDownUSDFText, data.showText)
	ClientTextUtils.setText(textUpUSDFText, data.showText)
