-- chunkname: @C:\\dev\\jkroot\\ws\\workspace\\XWinPublish\\proj\\p4dir\\client\\LuaScripts\\Guis\\Panels\\Tips\\Items\\A1TipArea\\TowerResultWinItem.lua
...
local TowerResultWinItem = Class.LightClass("TowerResultWinItem", BaseQueueItem)
local Const = require("Common.Const.Const")

function TowerResultWinItem:onInit()
	self:setMaxLimit(1)

	self.uContainer = self.uWidget
end

function TowerResultWinItem:onUpdate()
	self:tryPopupItem()
	self:refreshRemainTime()
end

function TowerResultWinItem:tryPopupItem()
...
function TowerResultWinItem:onClearRunningList(force)
	self:clearRecycleRunningList(force)
end

function TowerResultWinItem:refreshRemainTime()
...
function TowerResultWinItem:initUContainer(data)
...
function TowerResultWinItem:renderItem(item, data)
