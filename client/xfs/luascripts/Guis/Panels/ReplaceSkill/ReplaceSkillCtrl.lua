-- chunkname: @C:\\dev\\jkroot\\ws\\workspace\\XWinPublish\\proj\\p4dir\\client\\LuaScripts\\Guis\\Panels\\ReplaceSkill\\ReplaceSkillCtrl.lua
...
local ReplaceSkillCtrl = Class.LightClass("ReplaceSkillCtrl", UICtrl)
local ClientTextUtils = require("Utils.ClientTextUtils")

ReplaceSkillCtrl.messages = {
...
function ReplaceSkillCtrl:onCreate(info)
	self.model:setCurSelectPetId(info.curSelectPetId)
	UICtrl.onCreate(self, info)
end

function ReplaceSkillCtrl:addListener()
	function self.view.btnCloseUButton.luaClick()
		self:dismiss()

		if self._openInfo.finishedCallback then
			self._openInfo.finishedCallback()
		end
	end
	...
	for idx = 1, Const.PET_ABILITY_PRESET_COUNT do
		...
		function buttonUButton.luaClick()
			self:enterSelectingSkillsMode(false)
			pg.global.ui.tips:showCommonInput(pg.getGameString("RENAME_TITLE"), function(newName)
				self:tryRenameAbilityGroup(newName)
			end)
		end
	end
end

function ReplaceSkillCtrl:getShowName(name, idx)
...
function ReplaceSkillCtrl:onShow()
...
function ReplaceSkillCtrl:tryRenameAbilityGroup(newName)
	self.model:tryRenameAbilityGroup(newName)
end

function ReplaceSkillCtrl:refreshEquippedSkill(idx)
...
function ReplaceSkillCtrl:refreshUnlockSkill(idx)
	local unlockSkillData = self.model:getPetUnlockSkill()

	self.view.listLearnUList:SetList(unlockSkillData)
end

function ReplaceSkillCtrl:setupEquippedSkill(button, index, data)
...
function ReplaceSkillCtrl:selectEquipmentSlot(index)
...
function ReplaceSkillCtrl:selectUnlockedSkillSlot(index)
...
function ReplaceSkillCtrl:enterSelectingSkillsMode(enable, curAbilityId)
...
function ReplaceSkillCtrl:equipSkillLuaEndDrag(data, dropWidget, oriButton)
	if dropWidget and oriButton then
		self.model:tryModifyAbility(dropWidget.dataFromUList.abilityType, oriButton.dataFromUList.abilityId)
	else
		self.model:tryModifyAbility(data.abilityType, self.model.EMPTY_ABILITY_ID)
	end
end

function ReplaceSkillCtrl:setupAllSKills(button, index, data)
	...
	if pg.game.setting:getShowDebugId() then
		ClientTextUtils.setText(skillName, skillName.text, tostring(data.abilityId))
	end
...
function ReplaceSkillCtrl:unlockSkillLuaEndDrag(data, dropWidget, rayBox)
...
function ReplaceSkillCtrl:getAllEquipmentSkillBtns()
	return self.view.listEquipmentUList:GetAllButtons()
end

function ReplaceSkillCtrl:getAllUnlockedSkillBtns()
	return self.view.listLearnUList:GetAllButtons()
end

function ReplaceSkillCtrl:switchToPlanByIdx(idx)
	self.model:setCurShowPlanIdx(idx)
	self:refreshWholeListByPresetId(idx)
end

function ReplaceSkillCtrl:refreshWholeListByPresetId(idx)
	self:refreshEquippedSkill(idx)
	self:refreshUnlockSkill(idx)
end

function ReplaceSkillCtrl:onPetPresetChange(info)
	local presetId = info.presetId
	local presetInfo = self.model:getPetsAbilityPlanInfo(presetId)
	local presetName = self:getShowName(presetInfo.name, presetId)
...
function ReplaceSkillCtrl:continueSwitchPlanPages(isRight)
...
function ReplaceSkillCtrl:deSelectAllGamePadFocus()
...
function ReplaceSkillCtrl:initEquipSkillArea(data)
	local t = {}
	...
	for i = 1, #data do
		...
		if t[x][y].ValidSlot then
			...
			t[x][y].Fun1IsImportant = true
		end

		t[x][y].Fun2 = function(x1, y1)
			local btn = self.view.planBtns[self.model.curShowPlanIdx]
			local objectReference = btn:GetComponent("ObjectReference")
			local buttonUButton = objectReference:GetRefValue("buttonUButton")

			buttonUButton:OnClickSimulate()
		end
		t[x][y].Fun2Name = pg.getGameString("RENAME_SKILL_GROUP")
...
function ReplaceSkillCtrl:initUnlockSkillArea(data)
	local t = {}
...
function ReplaceSkillCtrl:equipSkillGamePadBeginDrag(button, data, cloneButton)
...
function ReplaceSkillCtrl:unlockSkillGamePadBeginDrag(button, data, cloneButton)
	local objectReference1 = cloneButton:GetComponent("ObjectReference")
	local skillName1 = objectReference1:GetRefValue("skillName")
	local powerNum1 = objectReference1:GetRefValue("powerNum")
	local costNum1 = objectReference1:GetRefValue("costNum")
...
function ReplaceSkillCtrl:onInputDeviceChanged(deviceType)
	return
end

function ReplaceSkillCtrl:getEquipListItemByCursorIndex(x, y)
	local btns = self.view.listEquipmentUList:GetAllButtons()
...
function ReplaceSkillCtrl:getUnlockListItemByCursorIndex(x, y)
	local btns = self.view.listLearnUList:GetAllButtons()
	local index = (x - 1) * 2 + y - 1
...
function ReplaceSkillCtrl:resetDragState()
	local btns = self.view.listEquipmentUList:GetAllButtons()
