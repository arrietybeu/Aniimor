-- chunkname: @C:\\dev\\jkroot\\ws\\workspace\\XWinPublish\\proj\\p4dir\\client\\LuaScripts\\Guis\\Panels\\PetChangeForm\\PetChangeFormModel.lua

local logger = require("Core.Log.LoggerManager").getLogger("PetChangeFormModel")
...
local PetChangeFormModel = Class.LightClass("PetChangeFormModel", UIModel)

function PetChangeFormModel:getNeedItemByTemplateId(templateId)
...
function PetChangeFormModel:getRealCostData(templateId)
	local petFormChangeInfo = PetFormChangeData[templateId]
...
function PetChangeFormModel:getIsCatched(templateId)
	if pg.me then
...
function PetChangeFormModel:getAllTargetInfo(fromTemplateId)
...
function PetChangeFormModel:getPetDataList(petId)
