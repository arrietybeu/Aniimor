-- chunkname: @C:\\dev\\jkroot\\ws\\workspace\\XWinPublish\\proj\\p4dir\\client\\LuaScripts\\SDK\\Platform\\EntityMixin\\ImpPlatformNormalPhotoUIComponent.lua
	...
	for _, entity in pairs(allEntitys) do
		if Utils.isPlayer(entity) and not entity.isMainPlayer and entity.active and entity.visible and entity.uid ~= nil and M.isFriendPlayerUid(entity.uid) then
			...
			if entPos then
				...
				if M.checkInViewportMaster(entPos, height) and not isBlock then
...
function M:savePhoto(traitId)
	...
	pg.global.eventEmitter:emit(EventConst.PLATFORM_ACHIEVEMENT_PHOTO_SAVED, {
		templateIds = self.curTemplateIds or {},
		hasPlayerInView = M.checkMainPlayerInViewport(),
		sceneId = pg.me and pg.me.space and pg.me.space.sceneId or 0,
		photoType = self.curPhotoType,
