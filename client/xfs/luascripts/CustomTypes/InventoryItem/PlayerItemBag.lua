-- chunkname: @C:\\dev\\jkroot\\ws\\workspace\\XWinPublish\\proj\\p4dir\\client\\LuaScripts\\CustomTypes\\InventoryItem\\PlayerItemBag.lua
...
local PlayerItemBag = class.LiteClass("PlayerItemBag", CustomDict)
local table_sort = table.sort

function PlayerItemBag:ctor()
	PlayerItemBag.super.ctor(self)
...
function PlayerItemBag:getCount()
	return self.count
end

function PlayerItemBag:getCapacity()
	return self.capacity
end

function PlayerItemBag:hasSpace(n)
	return self.count + n <= self.capacity
end

function PlayerItemBag:getSpace()
...
function PlayerItemBag:isFull()
	return not self:hasSpace(1)
end

function PlayerItemBag:isInvalidItem(item)
...
function PlayerItemBag:get(genID)
...
function PlayerItemBag:getByFilter(filter)
	local items = {}
...
function PlayerItemBag:getItemsById(id)
...
function PlayerItemBag:getItemsByIdWithBind(id, isBind)
	if not isBind then
		return {}
	end
...
function PlayerItemBag:__toArraBy(sort_function)
	local items = {}
...
function PlayerItemBag:toArrayById()
	return self:__toArraBy(__sort_by_genID)
end

function PlayerItemBag:toArrayByCfgId()
	return self:__toArraBy(__sort_by_id)
end

function PlayerItemBag:getAll()
	return self:__toArraBy(nil)
end

function PlayerItemBag:calcIdNumMap(idNumMap)
...
function PlayerItemBag:calcIdNumWithBindMap(idNumWithBindMap, onItem)
	local summary = ItemSummary()
...
function PlayerItemBag:__calcSpace(items, allowMultiPile)
	return ItemPileSimulator.calcSpace(self, items, allowMultiPile)
end

function PlayerItemBag:canHoldItems(items)
