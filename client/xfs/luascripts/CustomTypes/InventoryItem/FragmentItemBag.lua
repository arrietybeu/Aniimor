-- chunkname: @C:\\dev\\jkroot\\ws\\workspace\\XWinPublish\\proj\\p4dir\\client\\LuaScripts\\CustomTypes\\InventoryItem\\FragmentItemBag.lua
...
local FragmentItemBag = class.LiteClass("FragmentItemBag", CustomDict)
local table_sort = table.sort

function FragmentItemBag:ctor()
	FragmentItemBag.super.ctor(self)
...
function FragmentItemBag:getCount()
	return self.count
end

function FragmentItemBag:getCapacity()
	return self.capacity
end

function FragmentItemBag:hasSpace(n)
	return self.count + n <= self.capacity
end

function FragmentItemBag:getSpace()
...
function FragmentItemBag:isFull()
	return not self:hasSpace(1)
end

function FragmentItemBag:isInvalidItem(item)
...
function FragmentItemBag:get(genID)
...
function FragmentItemBag:getByFilter(filter)
	local items = {}
...
function FragmentItemBag:getItemsById(id)
...
function FragmentItemBag:getItemsByIdWithBind(id, isBind)
	if not isBind then
		return {}
	end
...
function FragmentItemBag:__toArraBy(sort_function)
	local items = {}
...
function FragmentItemBag:toArrayById()
	return self:__toArraBy(__sort_by_genID)
end

function FragmentItemBag:toArrayByCfgId()
	return self:__toArraBy(__sort_by_id)
end

function FragmentItemBag:getAll()
	return self:__toArraBy(nil)
end

function FragmentItemBag:calcIdNumMap(idNumMap)
...
function FragmentItemBag:calcIdNumWithBindMap(idNumWithBindMap, onItem)
	local summary = ItemSummary()
...
function FragmentItemBag:__calcSpace(items, allowMultiPile)
	return ItemPileSimulator.calcSpace(self, items, allowMultiPile)
end

function FragmentItemBag:canHoldItems(items)
